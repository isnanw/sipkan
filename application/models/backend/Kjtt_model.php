<?php
class Kjtt_model extends CI_Model
{
    /**
    ...
    {
        $this->db->insert('tb_kjtt', $data);
        $id = $this->db->insert_id();
        return (isset($id)) ? $id : FALSE;
    }
    public function tambah_detail($data1)
    {
        $this->db->insert('tb_kjttpp', $data1);
    }
    ...
    {
        return $this->db->update('tb_kjtt', $data, array('id' => $id));
    }

    public function update_detail($id, $data2)
    {
        return $this->db->update('tb_kjttpp', $data2, array('id_kjtt' => $id));
    }

    public function single_entry($id)
    {
        $this->db->select('*');
        $this->db->from('tb_kjtt');
        $this->db->where('id', $id);
        $query = $this->db->get();
    ...
    function delete($id)
    {
        return $this->db->delete('tb_kjtt', array('id' => $id));
    }

    function delete_detail($id)
    {
        return $this->db->delete('tb_kjttpp', array('id_kjtt' => $id));
    }


    function import($data)
    {
        $insert = $this->db->insert_batch('tb_kjtt', $data);
        if ($insert) {
            return true;
        }
    }
    public function edit($id)
    {
        $query = "SELECT ts.id,ts.lokasi as kodelokasi,l.lokasi,ts.kampung as kodekampung,
                    ...
                    LEFT JOIN tb_kjttpp ts2 ON ts2.id_kjtt = ts.id
                    WHERE ts.id = $id";
