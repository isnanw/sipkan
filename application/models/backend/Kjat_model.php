<?php
class Kjat_model extends CI_Model
{
    /**
    ...
    {
        $this->db->insert('tb_kjat', $data);
        $id = $this->db->insert_id();
        return (isset($id)) ? $id : FALSE;
    }
    public function tambah_detail($data1)
    {
        $this->db->insert('tb_kjatpp', $data1);
    }
    ...
    {
        return $this->db->update('tb_kjat', $data, array('id' => $id));
    }

    public function update_detail($id, $data2)
    {
        return $this->db->update('tb_kjatpp', $data2, array('id_kjat' => $id));
    }

    public function single_entry($id)
    {
        $this->db->select('*');
        $this->db->from('tb_kjat');
        $this->db->where('id', $id);
        $query = $this->db->get();
    ...
    function delete($id)
    {
        return $this->db->delete('tb_kjat', array('id' => $id));
    }

    function delete_detail($id)
    {
        return $this->db->delete('tb_kjatpp', array('id_kjat' => $id));
    }


    function import($data)
    {
        $insert = $this->db->insert_batch('tb_kjat', $data);
        if ($insert) {
            return true;
        }
    }
    public function edit($id)
    {
        $query = "SELECT ts.id,ts.lokasi as kodelokasi,l.lokasi,ts.kampung as kodekampung,
                            l2.lokasi as kampung,ts.ketua,ts.jml_anggota,ts.jml_unit,ts.jml_petak,
                    ...
                    LEFT JOIN tb_kjatpp ts2 ON ts2.id_kjat = ts.id
                    WHERE ts.id = $id";
