...
$(document).ready(function() {
    ...
    table = $('#mytable').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/pettycashdirektur/get_ajax_list')?>",
            "type": "POST",
            "data": function ( data ) {
...
function edit_pettycashdirektur(id_pettycash)
{
    save_method = 'update';
    $('#formpettycash')[0].reset();
    $('.form-group').removeClass('has-error');
    ...
    $.ajax({
        url : "<?php echo site_url('backend/pettycashdirektur/ajax_edit/')?>/" + id_pettycash,
        type: "GET",
        dataType: "JSON",
...
function addpettycash()
{
    ...
    var catatan_direktur = $("#catatandirektur").val();

    var fd = new FormData();
    fd.append("id", id);
    fd.append("pembahasan", status);
    fd.append("catatandirektur", catatan_direktur);
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('backend/pettycashdirektur/add')?>";
    } else {
        url = "<?php echo site_url('backend/pettycashdirektur/edit')?>";
    }
...
$(document).on("click", "#del", function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "post",
                    url : "<?php echo site_url('backend/pettycashdirektur/delete')?>",
                    data: {
                        id: id,
                    },
                    dataType: "json",
