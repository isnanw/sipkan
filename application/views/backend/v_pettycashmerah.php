...
<?php
    ...
    $no++;
?>
<div class="modal fade" id="galleryModal<?php echo $row->id_pettycash; ?>" tabindex="-1" role="dialog"
...
$(document).ready(function() {
    ...
    table = $('#mytable').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/pettycashmerah/get_ajax_list')?>",
            "type": "POST",
            "data": function ( data ) {
...
function cetakpdf()
{
    ...
    window.open("<?php echo base_url();?>backend/pettycashmerah/pdf/"+kar, "_blank");
}
...
function cetakexcel()
{
    var kar = $("#id_user_pettycash").val();
    var base_url = '<?php echo base_url(); ?>';
    window.open("<?php echo base_url();?>backend/pettycashmerah/excel/"+kar, "_blank");
}
...
function edit_pettycash(id_pettycash)
{
    ...
    $('.help-block').empty();
    $('#modal_form_pettycash').modal('show');
    $('.modal-title').text('Edit pettycash');
    var base_url = '<?php echo base_url(); ?>';
    $.ajax({
        url : "<?php echo site_url('backend/pettycashmerah/ajax_edit/')?>/" + id_pettycash,
        type: "GET",
        dataType: "JSON",
...
function addpettycash()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('backend/pettycashmerah/add')?>";
    } else {
        url = "<?php echo site_url('backend/pettycashmerah/edit')?>";
    }
...
$(document).on("click", "#del", function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "post",
                    url : "<?php echo site_url('backend/pettycashmerah/delete')?>",
                    data: {
                        id: id,
                    },
                    dataType: "json",
