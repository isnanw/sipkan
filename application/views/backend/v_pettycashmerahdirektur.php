...
$(document).ready(function() {
    ...
    table = $('#mytable').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/pettycashmerahdirektur/get_ajax_list')?>",
            "type": "POST",
            "data": function ( data ) {
...
function edit_pettycashmerahdirektur(id_pettycash)
{
    save_method = 'update';
    $('#formpettycash')[0].reset();
    $('.form-group').removeClass('has-error');
    ...
    $.ajax({
        url : "<?php echo site_url('backend/pettycashmerahdirektur/ajax_edit/')?>/" + id_pettycash,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="catatandirektur"]').val(data.catatan_direktur_bonmerah);
...
function addpettycash()
{
    ...
    var catatan_direktur_bonmerah = $("#catatandirektur").val();

    var fd = new FormData();
    fd.append("id", id);
    fd.append("pembahasan", status);
    fd.append("catatandirektur", catatan_direktur_bonmerah);
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('backend/pettycashmerahdirektur/add')?>";
    } else {
        url = "<?php echo site_url('backend/pettycashmerahdirektur/edit')?>";
    }
...
$(document).on("click", "#del", function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "post",
                    url : "<?php echo site_url('backend/pettycashmerahdirektur/delete')?>",
                    data: {
                        id: id,
                    },
                    dataType: "json",
