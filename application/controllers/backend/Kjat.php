<?php
class Kjat extends CI_Controller
{
    /**
    ...
    public function index()
    {
        ...
        $this->load->view('backend/v_kjat', $data);
    }
    ...
    public function v_input()
    {
        ...
        $this->load->view('backend/menu', $data);
        $this->load->view('backend/v_kjatinput', $data);
    }
    public function v_edit()
    {
        ...
        $this->load->view('backend/menu', $data);
        $this->load->view('backend/v_kjatedit', $data);
    }
    public function ajax_edit($idedit)
    {
        $data = $this->kjat_model->edit($idedit);
        echo json_encode($data);
    }

    public function get_ajax_list()
    {
        $list = $this->kjat_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $d) {
            $no++;
      ...
      <a class="dropdown-item" href="javascript:void()" title="Hapus" id="deletets" value="' . $d->id . '"><i class="bi bi-trash"></i> Hapus</a></div></div></div>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->kjat_model->count_all(),
            "recordsFiltered" => $this->kjat_model->count_filtered(),
            "data" => $data,
        );
    ...
    function add()
    {
        ...
        $idedit = $this->kjat_model->tambah($data);
        $data1 = array(
            'id_kjat' => $idedit,
        ...
        $prosesdetail = $this->kjat_model->tambah_detail($data1);
        if ($idedit) {
            // INSERT LOG
    ...
    function edit()
    {
        ...
        $idedit = $this->kjat_model->update($id, $data);
        $data1 = array(
            'jan' => $this->input->post('jan'),
            'feb' => $this->input->post('feb'),
            'mar' => $this->input->post('mar'),
        ...
        $prosesdetail = $this->kjat_model->update_detail($id, $data1);
        // INSERT LOG
    ...
    public function deletets()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->kjat_model->delete($iddelete) && $this->kjat_model->delete_detail($iddelete)) {
