<?php
class Rl extends CI_Controller
{
    /**
    ...
    public function v_input()
    {
        ...
        $data['site_favicon'] = $site['site_favicon'];
        $data['images'] = $site['images'];
        $data['title'] = 'Input Budidaya Rumput Laut ( Longline RL )';
        $data['title0'] = 'Produksi Budidaya Ikan';

        $this->load->view('backend/menu', $data);
        $this->load->view('backend/v_rlinput', $data);
    }
    ...
    function add()
    {
        ...
        $idedit = $this->rl_model->tambah($data);
        $data1 = array(
            'id_rl' => $idedit,
        ...
        $prosesdetail = $this->rl_model->tambah_detail($data1);
        if ($idedit) {
            // INSERT LOG
            $b = '<b>' . $nama_users . '</b> Melakukan Tambah RL';
            $data2 = array(
                'ket' => $b,
            );
