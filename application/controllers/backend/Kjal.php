<?php
class Kjal extends CI_Controller
{
    /**
    ...
    function __construct()
    {
        ...
        $this->load->model('backend/Kjal_model', 'kjal_model');
    ...
    public function v_input()
    {
        ...
        $data['images'] = $site['images'];
        $data['tahun'] = $site['tahun'];
        $data['title'] = 'Input Budidaya Ikan Jaring Apung Laut ( KJA L )';
        $data['title0'] = 'Produksi Budidaya Ikan';

        $this->load->view('backend/menu', $data);
        $this->load->view('backend/v_kjalinput', $data);
    }
    public function v_edit()
    {
        ...
        $data['images'] = $site['images'];
        $data['tahun'] = $site['tahun'];
        $data['title'] = 'Edit Budidaya Ikan Jaring Apung Laut ( KJA L )';
        $data['title0'] = 'Produksi Budidaya Ikan';

        $this->load->view('backend/menu', $data);
        $this->load->view('backend/v_kjaledit', $data);
    }
    public function ajax_edit($idedit)
    {
        $data = $this->kjal_model->edit($idedit);
        echo json_encode($data);
    }

    public function get_ajax_list()
    {
        $list = $this->kjal_model->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $d) {
            $no++;
      ...
      <a class="dropdown-item" href="' . base_url('backend/Kjal/v_edit/') . $d->id . '" title="Edit" ><i class="bi bi-pen-fill"></i> Edit</a>
      <a class="dropdown-item" href="javascript:void()" title="Hapus" id="deletets" value="' . $d->id . '"><i class="bi bi-trash"></i> Hapus</a></div></div></div>';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->kjal_model->count_all(),
            "recordsFiltered" => $this->kjal_model->count_filtered(),
            "data" => $data,
        );
    ...
    function add()
    {
        ...
        $idedit = $this->kjal_model->tambah($data);
        $data1 = array(
            'id_kjal' => $idedit,
        ...
        $prosesdetail = $this->kjal_model->tambah_detail($data1);
        if ($idedit) {
            // INSERT LOG
            $b = '<b>' . $nama_users . '</b> Melakukan Tambah KJAL';
            $data2 = array(
                'ket' => $b,
            );
    ...
    function edit()
    {
        ...
        $idedit = $this->kjal_model->update($id, $data);
        $data1 = array(
            'jan' => $this->input->post('jan'),
            'feb' => $this->input->post('feb'),
            'mar' => $this->input->post('mar'),
        ...
        $prosesdetail = $this->kjal_model->update_detail($id, $data1);
        // INSERT LOG
        $b = '<b>' . $nama_users . '</b> Melakukan Update KJAL';
        $data2 = array(
            'ket' => $b,
        );
        $this->jenisikan_model->insert_log_jenisikan($data2);
    ...
    public function deletets()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->kjal_model->delete($iddelete) && $this->kjal_model->delete_detail($iddelete)) {
